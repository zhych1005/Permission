...
<div id="dialog-cus-form" style="display: none;" >
    <form id="cusForm">
        <table class="table table-striped table-bordered table-hover dataTable no-footer" role="grid">
            ...
            <tr>
                <td><label for="grundlagen">获知途径</label></td>
                <td>
                    <select name="grundlagen" id="grundlagen" data-placeholder="获知途径" style="width: 150px;">
                        <option value="" disabled selected hidden>选择</option>
            ...
            <tr>
                ...
                <td><label for="timeOfVisit">来访日期</label></td>
                <td><input type="date" name="timeOfVisit" id="timeOfVisit" value="" /></td>
            </tr>

            <tr>
                <td><label for="acreage">面积</label></td>
                <td><input type="text" name="acreage" id="acreage" value=""/>㎡☀</td>
    ...
    <form id="callBackForm">
        <table class="table table-striped table-bordered table-hover dataTable no-footer" role="grid">
            <tr>
                ...
                <td><textarea name="remark" id="ca_remark" rows="3"
                              cols="25"></textarea></td>
            </tr>
        </table>
    </form>
    ...
    <form id="ch_cusForm">
        <table class="table table-striped table-bordered table-hover dataTable no-footer" role="grid">
            ...
            <tr>
                <td><label for="grundlagen">获知途径</label></td>
                <td>
                    <select name="grundlagen" id="c_grundlagen" data-placeholder="获知途径" style="width: 150px;">
                        <option value="" disabled selected hidden>选择</option>
                        <option value="路过">路过</option>
                        <option value="楼体">楼体</option>
                        <option value="朋介">朋介</option>
            ...
            <tr>
                ...
                <td><label for="timeOfVisit">来访日期</label></td>
                <td><input type="date" name="timeOfVisit" id="c_timeOfVisit" value="" /></td>
            </tr>

            <tr>
                <td><label for="acreage">面积</label></td>
                <td><input type="text" name="acreage" id="c_acreage" value=""/>㎡☀</td>
...
<%--电转访表单--%>
<div id="dialog-sh_cus-form" style="display: none;">
    <form id="sh_cusForm">
        <table class="table table-striped table-bordered table-hover dataTable no-footer" role="grid">
            <tr>
                <td><label for="cusname">客户姓名</label></td>
                <input type="hidden" name="id" id="sh_cusid" value=""/>
                <input type="hidden" name="userid" id="sh_userid" value=${user.id} />
                <input type="hidden" name="deptId" id="sh_deptId" value=${user.deptId} />
                <input type="hidden" name="cusSource" id="cusSource" value="2" />
                <td><input type="text" name="cusname" id="sh_cusname" value="" readonly
                           class="text ui-widget-content ui-corner-all">

                    <input type="hidden" name="cussex" id="sh_cussex" value=""/>
                    <input type="hidden" name="custel" id="sh_custel" value=""/>
                    <input type="hidden" name="cusPhone" id="sh_cusPhone" value=""/>
                    <input type="hidden" name="area" id="sh_area" value=""/>
                    <input type="hidden" name="focus" id="sh_focus" value=""/>
                    <input type="hidden" name="firstVisit" id="sh_grundlagen" value=""/>
                    <input type="hidden" name="cusstatus" id="sh_cusstatus" value=""/>
                </td>

                <td><label for="proxy">代理人</label></td>
                <td><input type="text" name="proxy" id="sh_proxy" value="" /></td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    <select name="familyStructure" id="familyStructure" data-placeholder="首访渠道" style="width: 150px">
                        <option value="" disabled selected hidden>选择</option>
                ...
                <td><input type="text" name="placeOfDomicile" id="placeOfDomicile" value="" /></td>
            </tr>
            ...
            <tr>
                ...
                <td><label for="remark">备注</label></td>
                <td><textarea name="remark" id="sh_remark" rows="3"
                              cols="25"></textarea></td>
            </tr>
    ...
    <td>
       <div class="hidden-sm hidden-xs action-buttons" style="text-align:center;">
           <a class="green cus-edit" href="#" data-id="{{id}}">
            ...
            <a class="cus-change" href="#" data-id="{{id}}">
                <i class="badge badge-success">电转访</i>
            </a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<script type="application/javascript">
    ...
    $(function () {
        ...
        function loadCusList() {
            ...
            var url = "/cusTel/form/addTelVisitPage.json?userid=${user.id}&deptId=${user.deptId}";
                ...
                $("#dialog-ch_cus-form").dialog({
                    ...
                    open: function (event, ui) {
                        ...
                        if (targetCus) {
                            ...
                            $("#id").val(targetCus.id);
                            $("#c_grundlagen").val(targetCus.grundlagen);
                            $("#c_acreage").val(targetCus.acreage);
                            $("#c_focus").val(targetCus.focus);

                            if (targetCus.timeOfVisit != null) {
                                $("#c_timeOfVisit").val(new Date(targetCus.timeOfVisit).Format("yyyy-MM-dd"));
                            } else {
                                return "";
                            }
                        }
        ...
        /*电转访表单*/
        function bindShCusClick() {
            $(".cus-change").click(function (e) {
                e.preventDefault();
                e.stopPropagation();
                var cusId = $(this).attr("data-id");
                $("#dialog-sh_cus-form").dialog({
                    modal: true,
                    title: "电访转来访信息完善",
                    open: function (event, ui) {
                        $(".ui-dialog-titlebar-close", $(this).parent()).hide();
                        ...
                        var targetCus = cusMap[cusId];
                        if (targetCus) {
                            /*$("#sh_cusid").val(targetCus.id);*/
                            $("#sh_cussex").val(targetCus.cussex);
                            $("#sh_custel").val(targetCus.custel);
                            $("#sh_cusPhone").val(targetCus.cusPhone);
                            $("#sh_cusname").val(targetCus.cusname);
                            $("#sh_area").val(targetCus.area);
                            $("#c_proxy").val(targetCus.proxy);
                            $("#sh_cusstatus").val(targetCus.cusstatus);
                            $("#c_remark").val(targetCus.remark);
                            /*$("#id").val(targetCus.id);*/
                            $("#sh_grundlagen").val(targetCus.grundlagen);
                            $("#sh_focus").val(targetCus.focus);
                        }
                    },
                    buttons: {
                        "转来访": function (e) {
                            e.preventDefault();
                            updateCus(false, function (data) {
                                $("#dialog-sh_cus-form").dialog("close");
                            }, function (data) {
                                showMessage("更新客户信息", data.msg, false);
                            })
                        },
                        "取消": function () {
                            $("#dialog-sh_cus-form").dialog("close");
        ...
        /*客户详情*/
        function bindcusInfoClick() {
            $(".cus-info").click(function (e) {
                e.preventDefault();
                e.stopPropagation();
                var cusId = $(this).attr("data-id");
                $("#dialog-cusInfo-form").dialog({
                    modal: true,
                    title: "客户详情",
                    open: function (event, ui) {
                        $(".ui-dialog-titlebar-close", $(this).parent()).hide();
                        ...
                        var targetCus = cusMap[cusId];
                        if (targetCus) {
                            $("#x_cusid").val(targetCus.id);
                            $("#x_cusname").val(targetCus.cusname);
                            $("#x_proxy").val(targetCus.proxy);
                            $("#x_cussex").val(targetCus.cussex);
                            $("#x_area").val(targetCus.area);
                            $("#x_custel").val(targetCus.custel);
                            $("#x_cusPhone").val(targetCus.cusPhone);
                            $("#x_cusstatus").val(targetCus.cusstatus);
                            $("#x_id").val(targetCus.id);
                            $("#x_grundlagen").val(targetCus.grundlagen);
                            $("#x_focus").val(targetCus.focus);
                            $("#x_acreage").val(targetCus.acreage);
                            $("#x_remark").val(targetCus.remark);

                            if (targetCus.timeOfVisit != null) {
                                $("#x_timeOfVisit").val(new Date(targetCus.timeOfVisit).Format("yyyy-MM-dd"));
                            } else {
                                return "";
                            }
                        }
                    ...
                    buttons: {
                        "关闭": function () {
                            $("#dialog-cusInfo-form").dialog("close");
                        }
                    }
                });
            });
        ...
        function updateUser(isCreate, successCallback, failCallback) {
            $.ajax({
                url:"/cusTel/form/updateTelCus.json",
        ...
        /*电转访的提交地址*/
        function updateCus(isCreate, successCallback, failCallback) {
            $.ajax({
                url:"/user/form/newForm.json",
                data: $("#sh_cusForm").serializeArray(),
                type: 'POST',
                success: function (result) {
                    if (result.ret) {
                        loadCusList();
        ...
        function addCus(isCreate, successCallback, failCallback) {
            $.ajax({
                url: "/cusTel/form/addTelCus.json",
        ...
        function callBackCus(isCreate, successCallback, failCallback) {
            $.ajax({
                url: "/TelCallBack/addTelCallBack.json",
